dSpider("mobile",function(session,env,$){
            ...
            require(["/i/service/model/fee_c3dfbd6.js","/i/apps/serviceapps/billdetail/billdetailone_288e057.js","/i/nresource/js/page/pages.js","btncommit","/i/apps/serviceapps/billdetail/billdetailtwo_00c3bc9.js"], function(fee, billdetailone, page, btn, billdetailtwo) {
                ...
                fee.getDetailInfo(req, function(data, total, start, end,time) {
                    ...
                    if (total != 0) {
                        ...
                        while (!push(session, {key : value})) {
                            push(session, {key : value});
                        };
    ...
    function checkSmsTime() {
        var time = session.get("mailCount");

        if (!time) {
            checkSmsTime();
            return;
        }
        window.firstSmsTm = parseInt(resp);
        alert('sec-' + window.firstSmsTm);
        window.firstSmsTmInterval = setInterval('xd_check()', 1000);
    }

    function xd_check() {
        console.log('等待短信中');
        if ((window.firstSmsTm + 60 * 1000) > new Date().getTime()) {
            clearInterval(window.firstSmsTmInterval);
            showMask(true);
    ...
    function showMask(isShow) {
        ...
        if (isShow) {
            if ($('#maskDiv').length == 0) {
                ...
                //发送短信
                var input = $('<input type="button" id="sendSmsBtn" value="免费获取验证码" onclick="settime(this)"/>');
                ...
                //认证
                var certificateBtn = $('<input type="button" id="certificateBtn" value="去认证" onclick="certificateBtnAction()"/>');
